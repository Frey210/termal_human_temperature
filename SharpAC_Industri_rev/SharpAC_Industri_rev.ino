...
void loop() {
  ...
  if (Serial.available() > 1) {
    String data = Serial.readStringUntil('\n');  // Membaca data dari koneksi Serial hingga karakter newline (\n)
    data.trim();                                 // Menghapus karakter whitespace di awal dan akhir string

    // Mengubah data suhu menjadi bilangan bulat
    suhu = data.toInt();
  }
